...
class DataCleaning:
    ...
    def clean_user_data(self):
        ...
        dropna_df = dropna_df[dropna_df['user_uuid'].str.len() ==36]
        ...
        dropna_df['user_uuid'] = dropna_df['user_uuid'].astype(str)
        max_length_index = dropna_df['user_uuid'].str.len().idxmax()
        longest_entry = dropna_df.loc[max_length_index, 'user_uuid']
        print(len(longest_entry))
        return dropna_df
    ...
    def clean_card_data(self):
        ...
        card_data['is_card_number_valid'] = card_data['card_number'].apply(lambda x: x.isdigit())

        #replace erroneous card numbers with NaN
        card_data.loc[~card_data['is_card_number_valid'], 'card_number'] = np.nan
        ...
        # Convert the list to a DataFrame with the same column name as in dim_card_details
        new_data = pd.DataFrame(card_numbers_to_add, columns=['card_number'])

        # For all other columns in dim_card_details, add NaNs to the new_data DataFrame
        for column in card_data.columns:
            if column not in new_data.columns:
                new_data[column] = np.nan  # Or use None, or an appropriate placeholder value

        # Append the new_data DataFrame to the existing dim_card_details DataFrame
        card_data = pd.concat([card_data, new_data], ignore_index = True)
    ...
    def clean_products_data(products_df):
        ...
        #Ensure uuid is the same length for all products (36 characters)
        products_df['uuid_length'] = products_df['uuid'].str.len()
        products_df = products_df[(products_df['uuid_length'] == 36)]
        #Return the cleaned dataframe
        return products_df
    ...
    def clean_orders_data(self):
        ...
        #Clean card_number column- replace any card numbers with non-numeric values with NaN
        # Flag rows with erroneous 'card_number' values
        table_df['card_number'] = table_df['card_number'].astype('str')
        table_df['is_card_number_valid'] = table_df['card_number'].apply(lambda x: x.isdigit())

        #replace erroneous card numbers with NaN
        table_df.loc[~table_df['is_card_number_valid'], 'card_number'] = np.nan

        
        #Find longest entry for database schema restrictions
